...
fn partition<T: PartialOrd + Copy>(data: &mut [T], pivot_index: usize) -> usize {
...
fn select_nth<T: PartialOrd + Copy>(data: &mut [T], n: usize) -> T {
    let mut left = 0;
...
pub fn median<T: PartialOrd + Copy + Add<Output = T> + Into<f64>>(data: &mut [T]) -> Option<f64> {
    if data.is_empty() {
        return None;
    }
    ...
    if len % 2 == 0 {
        ...
        Some((left_mid + right_mid).into() / 2.0)
    } else {
        // 奇数个元素：找出中间的数
        Some(select_nth(data, mid).into())
    }
}
