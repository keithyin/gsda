#!/usr/bin/env python3
"""Compact FastQ encoder/decoder - Version 3.
...
Per record:
  ...
  seq_packed : ceil(seq_len / 4) bytes
              ...
              Last byte right-padded with 00 if seq_len % 4 != 0.
  qual_hi    : ceil(seq_len / 2) bytes
              4 bits per qual (bits 5-2 of the 6-bit qual), MSB first.
              Last byte low nibble is 0 if seq_len is odd.
  qual_lo    : ceil(seq_len / 4) bytes
              2 bits per qual (bits 1-0 of the 6-bit qual), MSB first.
              Last byte right-padded with 00 if seq_len % 4 != 0.

Qual reconstruction:  qual_6bit = (qual_hi_val << 2) | qual_lo_val

Bytes per position:  seq=0.25 + qual_hi=0.5 + qual_lo=0.5 = 1.25 byte/base
(vs v2: 1 byte/base, v1: 0.5 + 0.5 = 1 byte/base)
"""
...
# ---------------------------------------------------------------------------
# Packing helpers - v3: 6-bit qual split into hi-4bit + lo-2bit, base separate
# ---------------------------------------------------------------------------
...
def pack_quals_v3(qual: bytes) -> tuple[bytes, bytes, int]:
    """Pack Phred+33 quals into separate hi-4bit and lo-2bit fields.

    Returns (qual_hi_packed, qual_lo_packed, clamped_count).
    """
    # Convert to raw 6-bit values
    raw = bytearray()
    clamped = 0
    for q in qual:
        v = q - 33
    ...
    # hi: 4 bits per value, 2 per byte, MSB first
    hi_out = bytearray()
    hi = True
    for v in raw:
        nibble = (v >> 2) & 0x0F       # bits 5-2
        if hi:
            hi_out.append(nibble << 4)
            hi = False
        else:
            hi_out[-1] |= nibble
            hi = True

    # lo: 2 bits per value, 4 per byte, MSB first
    lo_out = bytearray()
    acc, shift = 0, 6
    for v in raw:
        acc |= (v & 0x03) << shift     # bits 1-0
        if shift == 0:
            lo_out.append(acc)
            acc, shift = 0, 6
        else:
    ...
    return bytes(hi_out), bytes(lo_out), clamped
...
def unpack_quals_v3(qual_hi: bytes, qual_lo: bytes, seq_len: int) -> bytes:
    """Reconstruct Phred+33 quals from split hi/lo fields."""
    out = bytearray()
    for i in range(seq_len):
        # Extract hi nibble
        hi_byte_idx = i // 2
        if i % 2 == 0:
            hi_nibble = (qual_hi[hi_byte_idx] >> 4) & 0x0F
        else:
            hi_nibble = qual_hi[hi_byte_idx] & 0x0F

        # Extract lo 2 bits
        lo_byte_idx = i // 4
        lo_shift = 6 - 2 * (i % 4)
        lo_bits = (qual_lo[lo_byte_idx] >> lo_shift) & 0x03

        v = (hi_nibble << 2) | lo_bits
        out.append(v + 33)
    return bytes(out)
...
def encode(fastq_path: str | Path, out_path: str | Path, *, compress: bool = True) -> None:
    ...
    with opener(out_path, "wb") as fh:
        ...
        for name, seq, qual in records:
            ...
            qual_hi, qual_lo, n_clamped = pack_quals_v3(qual)
            clamped += n_clamped
            total_quals += len(qual)

            # header: name_len(2), seq_len(4), seq_packed_len(4), qual_hi_len(4), qual_lo_len(4)
            fh.write(struct.pack("<HIIII",
                                 len(name), len(seq),
                                 len(seq_packed), len(qual_hi), len(qual_lo)))
            fh.write(name)
            fh.write(seq_packed)
            fh.write(qual_hi)
            fh.write(qual_lo)
...
def decode(in_path: str | Path, out_path: str | Path) -> None:
    ...
    with opener(in_path, "rb") as fh, out_opener(out_path, "wb") as out:
        ...
        for _ in range(n_records):
            name_len, seq_len, seq_packed_len, qual_hi_len, qual_lo_len = struct.unpack(
                "<HIIII", fh.read(struct.calcsize("<HIIII")))
            name = fh.read(name_len)
            seq_packed = fh.read(seq_packed_len)
            qual_hi = fh.read(qual_hi_len)
            qual_lo = fh.read(qual_lo_len)

            seq = unpack_bases(seq_packed, seq_len)
            qual = unpack_quals_v3(qual_hi, qual_lo, seq_len)
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Encode FASTQ into a compact 2-bit base + split 6-bit qual binary format, "
