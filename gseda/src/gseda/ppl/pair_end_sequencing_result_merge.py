...
def main_cli():
    ...
    parser.add_argument("--intersect-only", action="store_true",
                        help="只输出两条 read 比对的重叠区（交集），而非拼接全长")
    ...
    mode = "intersect" if args.intersect_only else "merge"
    sum_fa = os.path.join(outdir, "intersect.fa" if args.intersect_only else "merged.fa")
    per_sample_suffix = ".intersect.fa" if args.intersect_only else ".fa"
    ...
    for sample in sorted(by_sample):
        ...
        if result is None:
            ...
            continue
        n_ok += 1
        out_seq = result["overlap"] if args.intersect_only else result["merged"]
