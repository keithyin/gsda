...
def main(
    ...
    env_prepare.check_and_install(
        "gsmm2-aligned-metric", semver.Version.parse("0.27.0"), "cargo install mm2")
...
def main_cli():
    ...
    parser.add_argument("--bams", nargs="+", type=str, default=None,
                        help="one or more raw (un-aligned) bam or fastq files")
    parser.add_argument("--refs", nargs="+", type=str, default=None,
                        help="reference genome fasta file(s), paired with --bams; a single ref is broadcast to all bams; omit when --disable-align-stat")
    parser.add_argument("--fact-csvs", nargs="+", type=str,
                        default=None, dest="fact_csvs",
                        help="pre-computed fact metric csv(s) to merge directly, skipping recomputation from bam")
    parser.add_argument("--basic-csvs", nargs="+", type=str,
                        default=None, dest="basic_csvs",
                        help="pre-computed basic metric csv(s) to merge directly, skipping recomputation from bam")

    parser.add_argument("--short-aln", type=int, default=0,
                        help="0/1 switch; set 1 for short-alignment mode when query or target length is in [30, 200]", dest="short_aln")

    parser.add_argument("--disable-basic-stat",
                        action="store_true", dest="disable_basic_stat",
                        help="skip basic stats (reads_num/n50/quality distribution), keep only alignment stats")
    parser.add_argument("--disable-align-stat",
                        action="store_true", dest="disable_align_stat",
                        help="skip alignment stats (identity/coverage), keep only basic stats")
