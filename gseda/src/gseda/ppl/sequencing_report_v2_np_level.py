...
def main(
    ...
    basic_dataframe = pl.read_csv(basic_metric_filename, separator="\t")
    aligned_dataframe = pl.read_csv(
        fact_metric_filename, separator="\t",  schema_overrides={"longIndel": pl.String})

    aligned_dataframe = aligned_dataframe.filter(pl.col("rname") != "")

    aligned_dataframe = aligned_dataframe.with_columns(row_align_span())

    joined_dataframe = basic_dataframe.join(
        aligned_dataframe, on="qname", how="inner")
